...
pub async fn test_lookup(server: &VaultServer, token: &str) {
    let resp = token::lookup(&server.client, token).await;
    assert!(resp.is_ok());
}

pub async fn test_lookup_accessor(server: &VaultServer, accessor: &str) {
    let resp = token::lookup_accessor(&server.client, accessor).await;
    assert!(resp.is_ok());
}

pub async fn test_lookup_self(server: &VaultServer) {
    let resp = token::lookup_self(&server.client).await;
    assert!(resp.is_ok());
}

pub async fn test_new(server: &VaultServer) {
    let resp = token::new(&server.client, None).await;
    assert!(resp.is_ok());
}

pub async fn test_new_orphan(server: &VaultServer) {
    let resp = token::new_orphan(&server.client, None).await;
    assert!(resp.is_ok());
}

pub async fn test_renew(server: &VaultServer, token: &str) {
    let resp = token::renew(&server.client, token, Some("20m")).await;
    assert!(resp.is_ok());
}

pub async fn test_renew_accessor(server: &VaultServer, accessor: &str) {
    let resp = token::renew_accessor(&server.client, accessor, Some("20m")).await;
    assert!(resp.is_ok());
}

pub async fn test_renew_self(server: &VaultServer) {
...
pub async fn test_revoke(server: &VaultServer, token: &str) {
    let resp = token::revoke(&server.client, token).await;
    assert!(resp.is_ok());
}

pub async fn test_revoke_accessor(server: &VaultServer, accessor: &str) {
    let resp = token::revoke_accessor(&server.client, accessor).await;
    assert!(resp.is_ok());
}

pub async fn test_revoke_orphan(server: &VaultServer, token: &str) {
    let resp = token::revoke_orphan(&server.client, token).await;
    assert!(resp.is_ok());
}

pub async fn test_revoke_self(server: &VaultServer) {
    let resp = token::revoke_self(&server.client).await;
    assert!(resp.is_ok());
}

pub async fn test_tidy(server: &VaultServer) {
...
mod role {
    ...
    pub async fn test_delete(server: &VaultServer, role_name: &str) {
        let resp = role::delete(&server.client, role_name).await;
        assert!(resp.is_ok());
    }

    pub async fn test_list(server: &VaultServer) {
        let resp = role::list(&server.client).await;
        assert!(resp.is_ok());
    }

    pub async fn test_read(server: &VaultServer, role_name: &str) {
        let resp = role::read(&server.client, role_name).await;
        assert!(resp.is_ok());
    }

    pub async fn test_set(server: &VaultServer, role_name: &str) {
        let resp = role::set(
            &server.client,
            role_name,
            Some(
...
async fn setup(server: &VaultServer) -> Result<Token, ClientError> {
    // Create a new token
    let resp = token::new(
        &server.client,
        Some(
